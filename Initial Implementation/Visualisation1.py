...
def update_bar_race(selected_sex, selected_departments):
    ...
    # Filtrer pour obtenir le top 15 prénoms chaque année
    def get_top_15(df):
    ...
    for year in years:
        ...
        frame = go.Frame(
            ...
            name=str(year),
            layout=go.Layout(
                title=f'Prénoms les plus populaires en {int(year)}',
            )
        )
        frames.append(frame)
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title=f'Prénoms les plus populaires en {int(years[0])}',
            xaxis=dict(title='Nombre de naissances', range=[0, initial_data['nombre'].max() * 1.1]),  # Ajouter un peu d'espace sur l'axe x
